...
abstract class Context {
  ...
  public abstract void filterObjectMove(Notification notif);
  public abstract void filterPersonMove(Notification notif);
...
class ParentNightOutFilter extends Context {
  ...
  public void filterApplianceStateChange(Notification notif) {
    if (notif.getPriorityLevel() == 1) {
      if (notif.getTag().equals("stove") || notif.getTag().equals("sink")) {
        notif.ttsText = notif.getNote().replace(":", "");
        queue.add(notif);
      } else if (notif.getTag().equals("wifi")) {
        notif.ttsText = "Wifi error";
        queue.add(notif);
      }
    } else if (notif.getPriorityLevel() >= 2) {
      notif.soundFile = getSamplePlayer("stoveOn.wav");
  ...
  public void filterApplianceStateChange(Notification notif) {
    if (notif.getFlag().equals("error")) {
      if (notif.getTag().equals("wifi")) {
        ...
        notif.ttsText = notif.getNote().replace(":", "");
        queue.add(notif);
      } else if (notif.getTag().equals("stove")) {
        wavePlayer();
